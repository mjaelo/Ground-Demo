...
func _apply_generation_result_defered(result: Dictionary) -> void:
	...
	# Backwards compatibility with older single-layer payloads.
	if transforms_by_mesh.is_empty() and result.has("transforms"):
		transforms_by_mesh[0] = result["transforms"]

	var loc: Vector2i = result.get("loc", Vector2i.ZERO)
	for mesh_id in transforms_by_mesh.keys():
		var transforms: Array = transforms_by_mesh[mesh_id]
		if transforms.size() == 0:
			continue
		if mesh_placement_manager and mesh_placement_manager._SCENE_MESHES.has(int(mesh_id)):
			mesh_placement_manager.spawn_scene_meshes(int(mesh_id), transforms, get_parent(), loc)
		else:
			var batch_size := 1000
			for i in range(0, transforms.size(), batch_size):
				var batch := transforms.slice(i, i + batch_size)
				terrain.instancer.add_transforms.call_deferred(int(mesh_id), batch)
